...
export default {
    methods:{
        ...
        takeoffButtonOnTouchStart(){
            ...
            let target_hold_duration = 5000; //ms
            this.interval = setInterval(() => {
                this.currentDuration = Math.abs(Date.now() - ini_time);
                this.progress = rangeScaler(this.currentDuration, 0, target_hold_duration, 0, 100);
            }, 25);
        },
...
<template>
    <div class="mobile_cmd_takeoff_btn_container">
        <BaseLabel id="takeoff_btn_acx" @contextmenu="onContextMenu" @touchstart="takeoffButtonOnTouchStart" @touchend="takeoffButtonOnTouchEnd" v-text="'Hold For Takeoff'"></BaseLabel>
